...
for i in range(1, math.ceil(total_information / 1000) + 1):
    ...
    data = json.loads(json_data)#json문자열을 파이썬 오브젝트로 변환

    for u in data['Corona19Status']['row']:
...
def coronaday(dayc):
    ...
    print(dayc, "  ->   총 확진자 수는 :", len(numdate), "명  입니다")
def cellectday():
    while True:
        dayz = input('날짜를 입력하세요(ex 2021-06-17):')
...
def coronaarea(area):
    ...
    print(area+ "의 확진자 수는 :", len(numarea), "명  입니다.")
...
def vaccination_graph():
    vaccination_num = pd.read_excel('C:/Users/User/PycharmProjects/pythonProject/covid_projects/Today_vaccination.xls',usecols=[0,1])
...
def machine_running_graph():
    vaccination_num = pd.read_excel(
        'C:/Users/User/PycharmProjects/pythonProject/covid_projects/Today_vaccination.xls',
        usecols=[0, 1]
    )
    between_data = (df['확진일']>='2021-02-26')&(df['확진일']<='2021-06-13')
    filtered_df = df[between_data]
